...
LogAI MCP Server transforms Claude Desktop into a stateful data analysis powerhouse by providing:
...
```bash
git clone https://github.com/yourusername/logai-mcp-server.git
cd logai-mcp-server
```
...
```json
{
  "mcpServers": {
    "logai": {
      "command": "python",
      "args": ["path/to/logai_mcp_server.py"],
      "env": {
        "PYTHONPATH": "path/to/logai-mcp-server"
      }
    }
  }
}
