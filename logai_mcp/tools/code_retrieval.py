...
from .treesitter_parser import Treesitter, LanguageEnum
...
def get_language_enum(language_str: str) -> Optional[LanguageEnum]:
...
class ExactCodeRetriever:
    ...
    def __init__(self, codebase_path: str, supported_languages: Optional[List[str]] = None):
    ...
    def find_method_implementation(self, method_name: str, class_name: Optional[str] = None) -> List[CodeImplementationResult]:
        """
    ...
    def find_class_implementation(self, class_name: str) -> List[CodeImplementationResult]:
        """
    ...
    def list_all_methods(self) -> List[Tuple[str, str, str]]:
        ...
        for file_path, language in self.file_list:
            ...
            try:
                ...
                for method_node in method_nodes:
                    methods.append((method_node.name, method_node.class_name or "", file_path))
...
def _find_method_implementation_impl(method_name: str, class_name: Optional[str] = None, 
                                   codebase_path: Optional[str] = None, 
                                   supported_languages: Optional[List[str]] = None) -> pd.DataFrame:
    """
    ...
    if not codebase_path:
        raise ValueError("Codebase path not configured. Set CODEBASE_PATH environment variable.")
    ...
    if not results:
        return pd.DataFrame(columns=['name', 'implementation', 'file_path', 'line_start', 'line_end', 
                                   'doc_comment', 'class_name', 'result_type'])
...
def _find_class_implementation_impl(class_name: str, codebase_path: Optional[str] = None,
                                  supported_languages: Optional[List[str]] = None) -> pd.DataFrame:
    """
...
def _list_all_methods_impl(codebase_path: Optional[str] = None,
                          supported_languages: Optional[List[str]] = None) -> pd.DataFrame:
    """
...
def _list_all_classes_impl(codebase_path: Optional[str] = None,
                          supported_languages: Optional[List[str]] = None) -> pd.DataFrame:
    """
...
def _get_codebase_stats_impl(codebase_path: Optional[str] = None,
                           supported_languages: Optional[List[str]] = None) -> pd.DataFrame:
    """
    ...
    if not stats:
        return pd.DataFrame(columns=['language', 'file_count', 'codebase_path', 'supported_languages'])
    ...
    logger.info(f"Codebase contains {sum(stats.values())} files across {len(stats)} languages")
    return df
...
if _codebase_path_available():
    ...
    async def configure_supported_languages(
        ...
        *,
        ...
        """
        valid_languages = {'java', 'kotlin', 'python', 'typescript', 'javascript', 'cpp', 'rust'}
    ...
    logger.info("Codebase path not detected - code retrieval tools will not be registered") 