...
async def preprocess_log_data(
    log_record: Union[str, LogRecordObject],
    custom_replace_list: List[List[str]],
    *,
    save_clean_as: str,
...
def _parse_log_data_impl(
    clean_logs: Union[pd.Series, pd.DataFrame, list[str]],
    ...
    if isinstance(clean_logs, pd.Series):
        ...
        raise TypeError(f"_parse_log_data_impl expects clean_logs to be Series, DataFrame, or list, got {type(clean_logs)}")
    ...
    try:
        ...
        logger.warning("Parsing failed: %s: %s. Falling back to original clean_logs.", type(exc).__name__, exc)
        parsed_loglines_series = pd.Series(logs_to_parse, dtype=str)

    if not isinstance(parsed_loglines_series, pd.Series):
        logger.error(f"Parsed result was not a Series (type: {type(parsed_loglines_series)}). Defaulting to Series from input logs.")
        parsed_loglines_series = pd.Series(_ensure_list_of_strings(logs_to_parse, logger), dtype=str)
...
async def parse_log_data(
    clean_logs: Union[str, pd.Series, pd.DataFrame, list[str]],
    parsing_algorithm: str = "drain",
    *,
    save_as: str,
):
