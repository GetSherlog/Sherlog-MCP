...
def _list_namespaces_impl() -> pd.DataFrame:
    ...
    try:
        ...
        if not namespaces.items:
            return pd.DataFrame(columns=['name', 'status', 'age_days', 'labels', 'annotations'])
...
def _list_pods_impl(namespace: str = "default", label_selector: Optional[str] = None) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if not pods.items:
            return pd.DataFrame(columns=['name', 'namespace', 'status', 'ready', 'restarts', 'age_days', 'node'])
...
def _get_pod_logs_impl(pod_name: str, namespace: str = "default", 
    ...
    try:
        ...
        logger.info(f"Retrieved {len(logs.splitlines())} lines of logs for pod {pod_name}")
        return logs
...
def _list_services_impl(namespace: str = "default") -> pd.DataFrame:
    ...
    try:
        ...
        if not services.items:
            return pd.DataFrame(columns=['name', 'namespace', 'type', 'cluster_ip', 'external_ip', 'ports', 'age_days'])
...
def _list_deployments_impl(namespace: str = "default") -> pd.DataFrame:
    ...
    try:
        ...
        if not deployments.items:
            return pd.DataFrame(columns=['name', 'namespace', 'ready', 'up_to_date', 'available', 'age_days'])
...
def _list_events_impl(namespace: str = "default", limit: int = 100) -> pd.DataFrame:
    ...
    try:
        ...
        if not events.items:
            return pd.DataFrame(columns=['namespace', 'last_seen', 'type', 'reason', 'object', 'message'])
...
def _get_pod_details_impl(pod_name: str, namespace: str = "default") -> pd.DataFrame:
    ...
    try:
        ...
        if pod.spec.containers:  # type: ignore
            for container in pod.spec.containers:  # type: ignore
                container_info = {
                    ...
                    'ports': [f"{p.container_port}/{p.protocol}" for p in (container.ports or [])],
                    'resources_requests': container.resources.requests if container.resources else None,
                    'resources_limits': container.resources.limits if container.resources else None
                }
        ...
        row = {
            ...
            'creation_timestamp': created.isoformat() if created else None,
            'start_time': pod.status.start_time.isoformat() if pod.status.start_time else None  # type: ignore
        }
...
def _list_nodes_impl() -> pd.DataFrame:
    ...
    try:
        ...
        if not nodes.items:
            return pd.DataFrame(columns=['name', 'status', 'roles', 'age_days', 'version', 'os', 'kernel'])
