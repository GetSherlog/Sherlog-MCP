...
def to_polars(df: Union[pd.DataFrame, pl.DataFrame, Any]) -> pl.DataFrame:
...
def to_pandas(df: Union[pd.DataFrame, pl.DataFrame, Any]) -> pd.DataFrame:
...
def create_dataframe_polars(data: Any) -> pl.DataFrame:
    ...
    logger.warning(f"Could not convert {type(data)} to polars DataFrame, returning empty DataFrame")
    return pl.DataFrame()


def create_dataframe_pandas(data: Any) -> Union[pl.DataFrame, pd.DataFrame, Any]:
...
def smart_create_dataframe(data: Any, prefer_polars: bool = True) -> Union[pl.DataFrame, pd.DataFrame]:
    ...
    if prefer_polars:
        try:
            ...
            logger.warning(f"Failed to create polars DataFrame: {e}, falling back to pandas")
...
__all__ = [
    'DataFrame', 'LazyFrame',
    'to_polars', 'to_pandas', 
    'smart_create_dataframe', 'read_csv_smart',
    'ensure_pandas_for_logai', 'optimize_for_analytics'
] 