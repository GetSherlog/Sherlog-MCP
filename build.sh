...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if ! docker info | grep -q "Username:"; then
        ...
        echo "echo \$GITHUB_TOKEN | docker login ghcr.io -u navneet-mkr --password-stdin"
        exit 1
    fi
    ...
    echo "✅ Successfully built and pushed $FULL_IMAGE_NAME:$NEW_VERSION"
    echo ""
    echo "To use the new image:"
    echo "docker run --rm -i \\"
    echo "  --volume=/var/run/docker.sock:/var/run/docker.sock \\"
    echo "  --mount=type=bind,src=/Users/navneetkumar/Documents/sherlog-data/,dst=/Users/navneetkumar/Documents/sherlog-data/,ro \\"
    echo "  $FULL_IMAGE_NAME:$NEW_VERSION"
else
    echo "Image built but not pushed. You can manually push with:"
