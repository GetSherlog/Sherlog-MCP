"""Sentry Tools for Sherlog MCP Server
...
def _list_projects_impl(organization_slug: str) -> pd.DataFrame:
    ...
    if not projects:
        return pd.DataFrame(
            columns=["id", "name", "slug", "platform", "status", "organization_slug"]
        )
...
def _get_sentry_issue_impl(organization_slug: str, issue_id: str) -> pd.DataFrame:
    ...
    url = urljoin(
        sentry_host, f"/api/0/organizations/{organization_slug}/issues/{issue_id}/"
    )

    logger.info(
        f"Fetching Sentry issue {issue_id} from organization: {organization_slug}"
    )
    response = session.get(url)
    response.raise_for_status()
    ...
    row = {
        ...
        "assigned_to": issue.get("assignedTo", {}).get("name")
...
def _list_project_issues_impl(
    ...
    url = urljoin(
        sentry_host, f"/api/0/projects/{organization_slug}/{project_slug}/issues/"
    )
    ...
    logger.info(
        f"Fetching Sentry issues for project: {organization_slug}/{project_slug}"
    )
...
def _get_sentry_event_impl(
    organization_slug: str, issue_id: str, event_id: str
) -> pd.DataFrame:
    ...
    url = urljoin(
        sentry_host,
        f"/api/0/organizations/{organization_slug}/issues/{issue_id}/events/{event_id}/",
    )
    ...
    row = {
        ...
        "release": event.get("release", {}).get("version")
...
def _list_issue_events_impl(
    organization_slug: str, issue_id: str, limit: int = 50
) -> pd.DataFrame:
    ...
    url = urljoin(
        sentry_host,
        f"/api/0/organizations/{organization_slug}/issues/{issue_id}/events/",
    )
...
def _resolve_short_id_impl(organization_slug: str, short_id: str) -> pd.DataFrame:
    ...
    url = urljoin(
        sentry_host, f"/api/0/organizations/{organization_slug}/shortids/{short_id}/"
    )
    ...
    logger.info(
        f"Resolved short ID {short_id} to issue: {result.get('issue', {}).get('title', 'No title')}"
    )
...
def _create_project_impl(
    organization_slug: str, team_slug: str, name: str, platform: str | None = None
) -> pd.DataFrame:
    ...
    url = urljoin(
        sentry_host, f"/api/0/teams/{organization_slug}/{team_slug}/projects/"
    )

    payload = {"name": name, "platform": platform or "other"}
...
def _list_organization_replays_impl(
    organization_slug: str,
...
if _sentry_credentials_available():
    ...
    async def list_projects(
        organization_slug: str, *, save_as: str
    ) -> pd.DataFrame | None:
        """List all accessible Sentry projects for a given organization.
    ...
    async def get_sentry_issue(
        organization_slug: str, issue_id: str, *, save_as: str
    ) -> pd.DataFrame | None:
        """Retrieve and analyze a Sentry issue by ID.
    ...
    @app.tool()
    async def get_sentry_event(
        organization_slug: str, issue_id: str, event_id: str, *, save_as: str
    ) -> pd.DataFrame | None:
        """Retrieve and analyze a specific Sentry event from an issue.
    ...
    async def list_issue_events(
        organization_slug: str, issue_id: str, limit: int = 50, *, save_as: str
    ) -> pd.DataFrame | None:
        """List events for a specific Sentry issue.
    ...
    async def resolve_short_id(
        organization_slug: str, short_id: str, *, save_as: str
    ) -> pd.DataFrame | None:
        """Retrieve details about an issue using its short ID.
