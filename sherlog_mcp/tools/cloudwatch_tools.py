...
def get_cloudwatch_logs_client():
    ...
    try:
        ...
        logger.info(
            f"Successfully initialized CloudWatch Logs client for region: {settings.aws_region}"
        )
...
def get_cloudwatch_client():
    ...
    try:
        ...
        logger.info(
            f"Successfully initialized CloudWatch client for region: {settings.aws_region}"
        )
...
if _aws_credentials_available():
    ...
    @app.tool()
    def list_log_groups(name_prefix: str | None = None, limit: int = 50) -> str:
    ...
    def list_log_streams(
        log_group_name: str, name_prefix: str | None = None, limit: int = 20
    ) -> str:
        ...
        try:
                ...
                log_streams.append(
                    {
                        ...
                        if stream.get("creationTime")
                        else None,
                        "first_event_time": datetime.fromtimestamp(
                            stream["firstEventTime"] / 1000
                        ).isoformat()
                        if stream.get("firstEventTime")
                        else None,
            ...
            logger.info(
                f"Listed {len(log_streams)} log streams in log group {log_group_name}"
            )
            return json.dumps(result, indent=2)
    ...
    def query_logs(
        ...
        try:
            ...
            logger.info(
                f"Executed CloudWatch Logs query, returned {len(results)} results"
            )
    ...
    def get_metric_statistics(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(formatted_datapoints)} datapoints for metric {namespace}/{metric_name}"
            )
            return json.dumps(result, indent=2)
    ...
    def get_alarm_history(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(history_items)} history items for alarm {alarm_name}"
            )
            return json.dumps(result, indent=2)
    ...
    def put_metric_data(namespace: str, metric_data: list[dict[str, Any]]) -> str:
        ...
        try:
            ...
            for metric in metric_data:
                formatted_metric = {"MetricName": metric["MetricName"]}
                ...
                if "Value" in metric:
                    ...
                    raise ValueError(
                        f"Metric {metric['MetricName']} must have either Value or StatisticValues"
                    )
                ...
                if "Timestamp" in metric:
                    if isinstance(metric["Timestamp"], str):
                        formatted_metric["Timestamp"] = parse_time_string(
                            metric["Timestamp"]
                        )
                    else:
            ...
            for i in range(0, len(formatted_metrics), 20):
                ...
                response = client.put_metric_data(Namespace=namespace, MetricData=batch)
            ...
            logger.info(
                f"Put {len(formatted_metrics)} metrics to CloudWatch namespace {namespace}"
            )
            return json.dumps(result, indent=2)
    ...
    logger.info(
        "AWS credentials not detected - CloudWatch tools will not be registered"
    )
