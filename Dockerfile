...
# Install ALL runtime dependencies AND the current project into the *system* site-packages layer
...
# Runtime still needs git binary if uv run (even without pyproject.toml) might check git deps.
# build-essential is removed as no compilation should happen at runtime.
RUN apt-get update \
...
# Re-create workdir and copy only the executable script
# NOT copying pyproject.toml here prevents uv run from creating a new venv at runtime
WORKDIR /app
COPY logai_mcp ./logai_mcp
COPY logai_mcp_server.py ./
